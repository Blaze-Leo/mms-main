...
const double Window::PROGRESS_REQUIRED_FOR_RUNTURN  = 33.33 * 1.5;
const double Window::PROGRESS_REQUIRED_FOR_BACKTURN = 33.33 * 2;
...
Window::Window(QWidget *parent) :
    ...
    createStat("Total 180 Turn", StatsEnum::COUNT_TURNBACK180, 7, 0, 7, 1, statsLayout);
    ...
    createStat("Total Left Run", StatsEnum::COUNT_CURVELEFT, 9, 0, 9, 1, statsLayout);
    createStat("Total Right Run", StatsEnum::COUNT_CURVERIGHT, 9, 2, 9, 3, statsLayout);
...
QString Window::executeCommand(QString command) {
    ...
    else if (function == "curveRight") {
        bool success = curveRight();
        return success ? "" : CRASH;
    }
    else if (function == "curveLeft") {
        bool success = curveLeft();
        return success ? "" : CRASH;
    }
...
void Window::updateMouseProgress(double progress) {
    ...
    else if (m_movement == Movement::TURN_BACK) {
    ...
    // Calculate the current translation and rotation for moving straight or turn 45 or 90 degree
    ...
    if (m_movement == Movement::RUN_RIGHT || m_movement == Movement::RUN_LEFT) {
        ...
        if (m_startingDirection == Direction::NORTH) {
            ...
            stopRun("Stopped running because of a wrong direction!");
            return;
        }
...
bool Window::wallDiagonal(int distance) {
    ...
    case Direction::SOUTHWEST:
        ...
        break;
    default:
        stopRun("Diagonal Run with wrong direction");
    }

    if(!isWithinMaze(posx,posy,true))
        return true;
    ...
    if(posx%2==1)
        return isWall({posx/2, posy/2, Direction::SOUTH});
    else
        return isWall({posx/2, posy/2, Direction::WEST});
...
bool Window::moveHalfForward(int distance) {
    ...
    if (wallHalfFront(moves)) {
        stopRun("Stopped running because of a wall when moving HALF FORWARD");
        return false;
    }
    moves += 1;
    }
...
void Window::turnBack() {
    m_movement = Movement::TURN_BACK;
    m_doomedToCrash = false;
    m_movesRemaining = 0;
...
bool Window::curveRight() {
    m_movement = Movement::RUN_RIGHT;
    m_doomedToCrash = wallRight();
    if(m_doomedToCrash)
    stopRun("Cannot Run Right");
    m_movesRemaining = 0;
    stats->addTurn(StatsEnum::COUNT_CURVERIGHT);
    if(real)
    m_speedSlider->setValue(m_sliderValue);
    return !m_doomedToCrash;
}

bool Window::curveLeft() {
    m_movement = Movement::RUN_LEFT;
    m_doomedToCrash = wallLeft();
    if(m_doomedToCrash)
    stopRun("Cannot Run Left");
    m_movesRemaining = 0;
    stats->addTurn(StatsEnum::COUNT_CURVELEFT);
    if(real)
    m_speedSlider->setValue(m_sliderValue);
    return !m_doomedToCrash;
}
